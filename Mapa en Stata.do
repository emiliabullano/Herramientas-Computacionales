...
* Hacemos el mapa.
spmap crimenescadamil using coord_ls, id(id) line(data(coord_ls) size(0.1) color(white)) label(label(crimenescadamil) xcoord(x_c) ycoord(y_c) size(vsmall)) clmethod(q) cln(6) title("Número de crímenes cada mil habitantes") legend(size(small) position(5) xoffset(15.05)) fcolor(Heat) plotregion(margin(b+15)) ndfcolor(white) name(g2,replace)  
